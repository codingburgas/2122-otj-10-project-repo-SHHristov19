...
namespace pm::dal
{
    ...
    namespace workLogManagement
    {
        ...
        vector<vector<string>> readDataFromWorkLogFile(string fileName, string idOfUser, vector<int>* identification)
        {
            vector<vector<string>> allData;
            ifstream file(fileName);
            string line;
        ...
        vector<string> getWorkLogDataById(string fileName, int idOfWorkLog)
        {
            vector<string> allData;
            ifstream file(fileName);
            string line;
            ...
            if (file.is_open())
            {
                while (getline(file, line))
                {
                    if (counter > -1)
                    {
                        ...
                        if (stoi(data.id) == idOfWorkLog)
                        {
                            allData.push_back(data.id);
                            allData.push_back(data.idOfTask);
                            allData.push_back(data.idOfUser);
                            allData.push_back(data.workTime);
                            allData.push_back(data.dataOfWorking);
                        }
                    }
                    counter++;
                }
